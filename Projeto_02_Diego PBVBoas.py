...
st.set_page_config(
    ...
    page_icon="https://github.com/diegobezzi/Pojeto_Previsao_Renda/main/ebac-course-utils/icon/favicon.ico",
    layout="wide",
...
<img src="https://raw.githubusercontent.com/diegobezzi/Pojeto_Previsao_Renda/main/ebac-course-utils/logo/newebac_logo_black_half.png" alt="ebac-logo" width=50%>
</div>

# **Profissão: Cientista de Dados**
### [**Projeto #02** | Previsão de renda](https://github.com/diegobezzi/Pojeto_Previsao_Renda)

**Por:** [Diego Perez Bezzi Vilas Boas](https://www.linkedin.com/in/diegobezzi/)<br>
**Data:** 24 de abril de 2024.<br>
...
st.markdown('# <div style="text-align:center"> [Previsão de renda](https://github.com/diegobezzi/Pojeto_Previsao_Renda) </div>',
            unsafe_allow_html=True)
...
filepath = 'C:\\Users\\User\\Desktop\\Anaconda Projects\\Modulo 16\\input\\previsao_de_renda.csv'
...
st.markdown('Analisando o pairplot, foi possível identificar alguns outliers na variável renda que podem afetar o resultado da análise de tendência, mesmo tendo baixa frequência. Em paralelo, também foi possível osbservar uma correlação baixa entre praticamente todas as variáveis quantitativas, reforçando os resultados obtidos na matriz de correlação.')
...
st.markdown('Atráves do gráfico *clustermap*, foi possível reforçar os resultados previamente analisados de baixa correlação com a variável `renda`. Somente a variável `tempo_emprego` apresenta um índice que podemos considerar útil para análise. Além disso, foram apresentadas duas variáveis booleanas, `posse_de_imovel` e `posse_de_veiculo`, mas que também possuem baixo índice de correlação com renda.')
...
st.markdown('Embora a correlação entre as variáveis tempo_emprego e renda não seja tão alta, foi possivel observar a covariância positiva com uma leve inclinação da linha de tendência')
...
with st.expander("Análise de relevância preditiva com variáveis booleanas", expanded=True):
    ...
    st.markdown('Comparando os gráficos acima, é possível verificar que a variável posse_de_veículo apresenta maior relevância na predição de renda, evidenciada pela maior distância entre os intervalos de confiança para aqueles que possuem e não possuem veículo. Já a variável posse_de_imóvel pelo contrário, não apresenta diferença significativa entre as possíveis condições de posse imobiliária.')
